...
(та же логика, что с ключами регионов). Исключения — словари `overrides`
региона и `cell_params`: туда можно добавить новый параметр, его имя
проверят регион и сборка расчёта.

Ключи вида `cell:ATP_i` пишутся в пути как есть:
`regions.0.overrides.cell:ATP_i=2.0`.
...
_OPEN_DICTS = ("overrides", "cell_params")   # сюда можно добавлять ключи
