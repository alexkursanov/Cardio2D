...
class SimulationConfig:
    ...
    def check(self) -> list[str]:
        ...
        # После преднагрузки зажимаются ВСЕ грани. Если механическая сетка
        # в одну ячейку по какой-то стороне, внутренних узлов нет: ткань
        # закреплена целиком и не деформируется вовсе (полная изометрия).
        mech = self.mesh.mechanical
        if mech.nx < 2 or mech.ny < 2:
            warnings.append(
                f"механическая сетка {mech.nx}×{mech.ny}: без внутренних узлов "
                f"зажатые после преднагрузки грани закрепляют ткань целиком — "
                f"деформации не будет; нужно хотя бы 2 ячейки по каждой стороне")
