...
| Документ | Для чего |
|----------|----------|
| [docs/STUDENT_GUIDE.md](docs/STUDENT_GUIDE.md) | **начать отсюда**: пошагово — сервер, окружение, первый расчёт, MPI, фон, серии, очередь, продолжение, неполадки |
| [docs/INPUT_FILES.md](docs/INPUT_FILES.md) | конфигурация (все поля), модели, регионы и ишемия, протоколы стимуляции, серии, файл очереди |
| [docs/OUTPUT_FILES.md](docs/OUTPUT_FILES.md) | папка прогона и серии: run.json, series.csv, activation.npz, снимки, XDMF, чекпоинты, логи |
| [docs/ANALYSIS_GUIDE.md](docs/ANALYSIS_GUIDE.md) | обработка: контроль качества, ноутбуки 03–05, Python API, ParaView, перенос данных |

Очередь расчётов: `python scripts/queue.py QUEUE.txt --np 8 --parallel 3`
(образец — `examples/queue_example.txt`).
...
| **11** | `models/cell/tnnpm.py` — TNNPM (без 2-комп. СР и CaMKII, с I_K(ATP)); параметры клетки по регионам; двусторонняя связь клетка ↔ ткань | **на проверке**, 15 + 11 | модель — нет |
...
Модель выбирается `cell_model: "tnnpm"` — вариант для ткани с двусторонней
связью: в клетке только сократительный элемент, пассивная механика — в
ткани; ткань задаёт клетке длину саркомера 1.67·λ_f и скорость, клетка
отдаёт силу, а сила–скорость учитывается неявно в равновесии.
`"tnnpm_isometric"` — полная схема оригинала без обратной связи. Параметры клетки по регионам —
...
В `notebooks/` — пять ноутбуков, открываются в VS Code (или Jupyter):
...
| `05_deformation.ipynb` | деформация: исходная сетка → преднагрузка → пик установившегося удара; смещения от преднагрузки (стрелки), раскадровка и анимация удара (GIF), выход на стационар по ударам, растяжение по регионам, сравнение стадий серии | нет |
...
examples/      примеры конфигураций, серий и очереди
scripts/       queue.py — очередь расчётов; night*.sh — ночные серии
docs/          руководства пользователя
```
